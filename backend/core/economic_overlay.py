...
from core.params import EconomicParams, SizingResult
...
def evaluate_costs(
    ...
    for _, row in feasible.iterrows():
        ...
        wind_mw  = float(row.get(CurveCols.WIND_MW,  0) or 0)
        ...
        capex_wind         = wind_mw  * eco.cost_wind_mw   # 0 when wind not used
        ...
        total_capex        = (capex_pv + capex_wind + capex_bess_mw
                              + capex_bess_mwh + capex_grid_conn)
        ...
        r["CAPEX Wind (€M)"]          = round(capex_wind / 1e6, 3)
