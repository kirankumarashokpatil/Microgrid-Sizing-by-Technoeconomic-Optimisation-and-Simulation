"""
Legacy parameter container — ARCHIVED.

`ProjectParams` is the combined parameter object used ONLY by the superseded
Phase-2 full pipeline in this folder (config_loader → dispatch_engine →
financials / kpi_engine → reporter). The live pipeline uses
`optimizer.params.PhysicalParams` + `EconomicParams` instead. Kept for reference.
"""
...
class ProjectParams:
    """
    Legacy combined parameter object used by the old Phase 2 full pipeline.
    Not used by the live Phase 1 / Phase 2 code.
    """
    # General Config
    horizon_hours: int
