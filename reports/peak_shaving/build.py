...
from optimizer.curve_runner import run_peak_shaving_curve  # noqa: E402
...
def build(profiles_path: Path, out_dir: Path, gc_step: float = 10.0) -> Path:
    df, _, pv = load_profiles(xlsx_path=profiles_path, dt_hours=1.0)
    p = PhysicalParams(dt_hours=1.0, gc_sweep_step_mw=gc_step)
    dt = p.dt_hours
    ...
    out = out_dir / "GC_PeakShaving_Report.xlsx"
    summary, made = [], {}
...
def main():
    ...
    a = ap.parse_args()
    build(Path(a.profiles), Path(a.out), a.gc_step)
